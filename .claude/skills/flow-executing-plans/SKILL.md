...
**After each numbered step and after each task batch, check context usage.**
...
### Step 0.5: Verify Workspace (Branch Check)

Before implementing, ensure you are on a feature branch:

1. Run `git branch --show-current` to see the current branch
2. If already on a feature branch: proceed
3. If on main/develop: **STOP** — ask the user to confirm which branch to work on, or create a new feature branch
...
1. Run all tests: `./gradlew :composeApp:testDebugUnitTest`
...
- **flow-unit-tests** — MUST be invoked before writing any test code when executing plan steps
- **Completion checklist** — run tests, push branch, open PR to develop
